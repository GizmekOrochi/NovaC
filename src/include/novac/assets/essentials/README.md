...
Essentials is the standard statement-building package provided by NovaC. While Atomic provides expressions such as literals and operators, Essentials provides the language constructs required to build complete imperative programming languages.

Essentials installs reusable language features on top of an existing Engine. It does not replace the Engine nor Atomic; instead, it extends them with statements, scopes, variables, functions, and control flow.
...
The standard implementation also provides a built-in `print()` function.
...
func main() {
    ...
    auto program = engine.parse(source);

    engine.validate(*program);

    auto result = engine.eval(*program);
...
* **Engine** provides parsing, AST infrastructure, validation, and runtime.
...
* Typing (type system and semantic analysis)
