...
void ImportResolver::exportSymbol(
...
const Module &ImportResolver::load(const std::string &name) {
    ...
    if (loading_.find(name) != loading_.end()) {
        throw std::runtime_error(
            "ImportResolver::load: circular import involving module '" + name + "'");
    }
    ...
    if (sourceIter == virtualFiles_.end()) {
        throw std::runtime_error(
            "ImportResolver::load: module not found '" + name + "'");
    }
    ...
    for (const Export &exportSymbol : module.exports) {
        if (exportSymbol.visibility == Visibility::Public) {
            module.names.symbols[exportSymbol.name] = {
                name,
                exportSymbol.name,
                exportSymbol.visibility
            };
        }
    }
    ...
    if (!loaded) {
        throw std::runtime_error(
            "ImportResolver::load: failed to cache module '" + name + "'");
    }
...
ImportedSymbol ImportResolver::resolveQualified(const std::string &moduleName, const std::string &symbol) {
    ...
    if (iter == module.names.symbols.end()) {
        throw std::runtime_error(
            "ImportResolver::resolveQualified: module '" + moduleName
            + "' does not export symbol '" + symbol + "'");
    }
