...
LanguageFeatureInfo BooleanLiteralFeature::info() const {
    return {"language.literal.boolean", "0.2.0", "Boolean literal syntax and runtime value", {"literal.boolean", "expression.atom", "runtime.value.boolean"}, {}, {}, {}, {}};
}
...
void BooleanLiteralFeature::install(LanguageOptionsController &language) const {
    ...
    language.node({.kind = kind, .fields = {{.name = "value", .kind = ast::FieldKind::Bool, .required = true}}, .traits = {"expr", "literal"}, .doc = "Boolean literal expression"});
    ...
    language.expression(kind, [](const ast::Node &node, runtime::RuntimeContext &) { return runtime::Value::boolean(std::get<bool>(node.field("value"))); });
}
