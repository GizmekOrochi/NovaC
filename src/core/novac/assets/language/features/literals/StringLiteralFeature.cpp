...
LanguageFeatureInfo StringLiteralFeature::info() const {
    return {"language.literal.string", "0.2.0", "String literal syntax and runtime value", {"literal.string", "expression.atom", "runtime.value.string"}, {}, {}, {}, {}};
}
...
void StringLiteralFeature::install(LanguageOptionsController &language) const {
    ...
    language.node({.kind = kind, .fields = {{.name = "value", .kind = ast::FieldKind::String, .required = true}}, .traits = {"expr", "literal"}, .doc = "String literal expression"});
    ...
    language.expression(kind, [](const ast::Node &node, runtime::RuntimeContext &) { return runtime::Value::string(node.str("value")); });
}
