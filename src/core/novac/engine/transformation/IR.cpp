#include "../../../../include/novac/engine/transformation/IR.hpp"
...
BlockId HIRBuilder::createBlock(std::string name) {
    const BlockId id{
        static_cast<std::uint32_t>(module_.blocks.size())
    };
...
void HIRBuilder::setCurrentBlock(BlockId block) {
    if (block.value >= module_.blocks.size()) {
        throw std::runtime_error(
            "HIRBuilder::setCurrentBlock: invalid block id");
    }

    currentBlock_ = block;
}

ValueId HIRBuilder::emitValue(
    std::string op,
...
BasicBlock &HIRBuilder::currentBlock() {
    if (currentBlock_.value >= module_.blocks.size()) {
        throw std::runtime_error(
            "HIRBuilder::currentBlock: invalid current block");
    }
...
void MIRBuilder::setCurrentBlock(BlockId block) {
    if (block.value >= module_.blocks.size()) {
        throw std::runtime_error(
            "MIRBuilder::setCurrentBlock: invalid block id");
    }

    currentBlock_ = block;
}

ValueId MIRBuilder::emitValue(
    std::string op,
    std::vector<Operand> operands,
    ValueType type) {
    const ValueId result{nextValue_++};
...
BasicBlock &MIRBuilder::currentBlock() {
    if (currentBlock_.value >= module_.blocks.size()) {
        throw std::runtime_error(
            "MIRBuilder::currentBlock: invalid current block");
    }
...
LoweringRegistry::LoweredValue LoweringRegistry::lowerHIR(
    const ast::Node &node,
    HIRBuilder &out) const {
    const auto iter{hir_.find(node.kind())};

    if (iter == hir_.end()) {
        throw std::runtime_error(
            "LoweringRegistry::lowerHIR: missing HIR lowerer for AST node kind '"
            + node.kind() + "'");
    }
...
LoweringRegistry::LoweredValue LoweringRegistry::lowerMIR(
    const Instruction &instruction,
    MIRBuilder &out) const {
    const auto iter{mir_.find(instruction.op)};

    if (iter == mir_.end()) {
        throw std::runtime_error(
            "LoweringRegistry::lowerMIR: missing MIR lowerer for HIR instruction '"
            + instruction.op + "'");
    }

    return iter->second(instruction, out, *this);
}
