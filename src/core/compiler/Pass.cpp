...
void PassManager::run(CompilationContext &context) const {
    for (const std::unique_ptr<Pass> &pass : passes_) {
        ...
        if (context.diagnostics().hasErrors()) {
            throw std::runtime_error(
                "PassManager::run: pass '" + pass->name()
...
void AstValidationPass::run(CompilationContext &context) const {
    const ast::NodePtr &root{
        context.requireArtifact<ast::NodePtr>(astArtifact_)
    };

    context.language().nodes.validate(*root);
}

HIRLoweringPass::HIRLoweringPass(
    std::string inputAst,
    std::string outputHir)
    : inputAst_{std::move(inputAst)},
      outputHir_{std::move(outputHir)} {}
...
void HIRLoweringPass::run(CompilationContext &context) const {
    const ast::NodePtr &root{
        context.requireArtifact<ast::NodePtr>(inputAst_)
    };
...
MIRLoweringPass::MIRLoweringPass(
    std::string inputHir,
    std::string outputMir)
    : inputHir_{std::move(inputHir)},
      outputMir_{std::move(outputMir)} {}
...
void MIRLoweringPass::run(CompilationContext &context) const {
    const ir::HIRModule &hir{
        context.requireArtifact<ir::HIRModule>(inputHir_)
    };
...
RuntimePass::RuntimePass(
    ...
    : inputAst_{std::move(inputAst)},
      outputValue_{std::move(outputValue)} {}
