#include "../../include/overload/Overload.hpp"
...
std::optional<RankedCandidate> OverloadRegistry::best(
    ...
    for (const Candidate &candidate : iter->second.candidates()) {
        ...
        for (std::size_t index{}; index < args.size(); ++index) {
            if (candidate.params[index]->display() == args[index]->display()) {
                score += 10;
            } else if (types.assignable(candidate.params[index], args[index])) {
                score += 1;
            } else {
                valid = false;
                break;
        ...
        if (score == bestCandidate->score) {
            throw std::runtime_error(
                "OverloadRegistry::best: ambiguous overload call '" + name + "'");
        }
    }
...
std::optional<RankedCandidate> OperatorRegistry::resolve(
    const std::string &op,
    const std::vector<types::TypeRef> &args,
    const types::TypeRegistry &types) const {
